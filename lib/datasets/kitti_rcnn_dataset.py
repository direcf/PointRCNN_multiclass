...
class KittiRCNNDataset(KittiDataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='Multiclass', mode='TRAIN', random_select=True,
        ...
        if cfg.RPN.ENABLED:
            if gt_database_dir is not None:
                #### pkl file loading method
                self.gt_database = pickle.load(open(gt_database_dir, 'rb'))
                print("4 : gt_database key print and easy_list, hard_list = ", self.gt_database[0].keys()) 
                #### dict_keys(['sample_id', 'cls_type', 'gt_box3d', 'points', 'intensity', 'obj'])
    ...
    def preprocess_rpn_training_data(self):
        ...
        for idx in range(0, self.num_sample):
            # print("1",self.num_sample) # 3712
            # print("2",self.image_idx_list) # train.txt
            sample_id = int(self.image_idx_list[idx])
            # print(self.get_label(sample_id)) => [<lib.utils.object3d.Object3d object at 0x7f7994c3be10>,...]
            obj_list = self.filtrate_objects(self.get_label(sample_id))
            if len(obj_list) == 0:
                # self.logger.info('No gt classes: %06d' % sample_id)
                continue
            self.sample_id_list.append(sample_id)
        print("5 : filter TRAIN results: 3687 / 3712 (filtering)")
    ...
    def __len__(self):
        if cfg.RPN.ENABLED:
            # print("--len--", len(self.sample_id_list)) # 3687
    ...
    def get_rpn_sample(self, index):
        ...
        if sample_id < 10000:
            ...
            pts_rect = calib.lidar_to_rect(pts_lidar[:, 0:3]) #### What does it mean?
        ...
        # print("rpn_cls_label", rpn_cls_label)
        sample_info['rpn_reg_label'] = rpn_reg_label
        # print("rpn_reg_label", rpn_reg_label)
        sample_info['gt_boxes3d'] = aug_gt_boxes3d
        return sample_info
    ...
    def apply_gt_aug_to_one_scene(self, sample_id, pts_rect, pts_intensity, all_gt_boxes3d):
        ...
        while try_times > 0:
            ...
            # print("new", new_gt_dict['gt_box3d']) # 7 = problem!
    ...
    def get_rcnn_sample_info(self, roi_info):
        ...
        # classification label #### catch you!
