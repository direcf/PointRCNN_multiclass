...
class RPN(nn.Module):
    def __init__(self, use_xyz=True, mode='TRAIN'):
        ...
        print("7 : backone_network == pointnet2_msg") #  self.backbone_net

        # classification branch
        cls_layers = []
        pre_channel = cfg.RPN.FP_MLPS[0][-1] # 128
        for k in range(0, cfg.RPN.CLS_FC.__len__()): # 0 (only one time)
        ...
        reg_layers = []
        pre_channel = cfg.RPN.FP_MLPS[0][-1] # 128
        for k in range(0, cfg.RPN.REG_FC.__len__()): # 0 (only one time)
        ...
        #### I think we don't use RPN.LOSS_CLS
        ...
        print("8 : proposal_layer")
        print("9 : 'rpn_cls', 'rpn_reg', 'backbone_xyz', 'backbone_features'")       
        self.init_weights()
