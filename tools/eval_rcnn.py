...
def save_kitti_format(sample_id, calib, bbox3d, kitti_output_dir, scores, img_shape, class_pred_selected):
...
#   [2.133492   0.17414272 1.7325555 ]]]
    ...
    # After
    # [[ 32.06710373 185.66581597 215.41528805 242.7931933 ]
    # [506.29744071 171.08994752 574.8651604  208.23139691]
    # [328.51843528 176.52153966 392.17077686 206.71395641]
    # [605.38849999 167.10029646 625.63506981 185.05050713]
    # [579.50679664 168.74244761 605.61354296 186.1269185 ]]
    ...
    with open(kitti_output_file, 'w') as f:
        for k in range(bbox3d.shape[0]):
            ...
            # annotation
            index = class_pred_selected[k].cpu().item()
            class_to_name = {1: 'Car', 2:'Pedestrian', 3:'Cyclist'}
            print('%s -1 -1 %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f' %
                  (class_to_name[index], alpha, img_boxes[k, 0], img_boxes[k, 1], img_boxes[k, 2], img_boxes[k, 3],
                   bbox3d[k, 3], bbox3d[k, 4], bbox3d[k, 5], bbox3d[k, 0], bbox3d[k, 1], bbox3d[k, 2],
                   bbox3d[k, 6], scores[k]), file=f) # 출력이 안된 이유가 이거였네...
            # print('%s -1 -1 %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f' %
...
def save_rpn_features(seg_result, rpn_scores_raw, pts_features, backbone_xyz, backbone_features, kitti_features_dir,
    ...
    output_file = os.path.join(kitti_features_dir, '%06d.npy' %oo)
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        inputs = torch.from_numpy(pts_input).cuda(non_blocking=True).float()
        input_data = {'pts_input': inputs}
        # print("input_data size", inputs.size()) # size([1,16384,3])
        ...
        # print("rcnn_cls", rcnn_cls.size()) # size([1,100,4]) #### 가장 높은 것을 cls로 선택
        ...
        if rcnn_cls.shape[2] == 1:
            ...
            raw_scores = rcnn_cls.max(dim=2)[0].unsqueeze(dim=2)
            # raw_scores = rcnn_cls[:,pred_classes]
            # print("raw_scores",raw_scores) # inlcuding minus value
            # print("raw_scores.size",raw_scores.size()) # size([1,100,4])
            
            norm_scores = cls_norm_scores.max(dim=2)[0].unsqueeze(dim=2)
            # norm_scores = cls_norm_scores[:, pred_classes] #### why does .long need?
            # print("norm_scores",norm_scores) # value range (0~1)
            # print("norm_scores.size",norm_scores.size()) # size([1,100,4])
            # print("rcnn cls", rcnn_cls)
        ...
        #             for idx, thresh in enumerate(thresh_list):
        #                 total_recalled_bbox_list[idx] += (gt_max_iou > thresh).sum().item()
        #             recalled_num += (gt_max_iou > 0.1).sum().item() # 0.7
        ...
        # print("norm scores size", norm_scores.size()) # size([1,100,4])
        # print(inds.size()) # size([1,100,4])

        cur_inds = []
        
        for k in range(batch_size): # only one time
            for i in range(100):
                if pred_classes[i] >= 1:
                    cur_inds.append(i)

            class_pred = pred_classes[cur_inds]
            # print(cur_inds)

            # cur_inds = inds[k].view(-1)
            # print("cur_inds",cur_inds) # size([100])
            # if cur_inds.sum() == 0:
            #     continue

            # print("pred_boxes3d",pred_boxes3d.size()) # size([1,100,7])
            pred_boxes3d_selected = pred_boxes3d[k, cur_inds]
            # print("pred_box", pred_boxes3d_selected) # size([100,7])
            # print("pred_boxes3d_selected", pred_boxes3d_selected) 

            raw_scores_selected = raw_scores[k, cur_inds]
            # print("raw_scores", raw_scores)
            # print("raw_scores_selected", raw_scores_selected)
            # print("before",raw_scores_selected) # size([100,4])
            # raw_scores_selected = raw_scores_selected.max(dim=1)[0]
            # print("after", raw_scores_selected)

            norm_scores_selected = norm_scores[k, cur_inds]
            # print("norm_scores",norm_scores) # size([N,1])
            ...
            # rotated nms
            boxes_bev_selected = kitti_utils.boxes3d_to_bev_torch(pred_boxes3d_selected)
            # print("boxes_bev_selected ",boxes_bev_selected.size()) # size([5~31, 5]) boxes_bev: (N, 5) [x1, y1, x2, y2, ry]
            keep_idx = iou3d_utils.nms_gpu(boxes_bev_selected, raw_scores_selected, cfg.RCNN.NMS_THRESH).view(-1)
            # print("cur_inds", cur_inds)
            # print("keep", keep_idx)
            class_pred_selected = class_pred[keep_idx] ##### class prediction
            # print(class_pred_selected)
            # print("keep idx", keep_idx) #### NMS로 한번더 선별
            # keep_idx = same_number(keep_idx)
            pred_boxes3d_selected = pred_boxes3d_selected[keep_idx]
            # print("pred_boxes3d_selected", pred_boxes3d_selected)
            scores_selected = raw_scores_selected[keep_idx]
            pred_boxes3d_selected, scores_selected = pred_boxes3d_selected.cpu().numpy(), scores_selected.cpu().numpy()
            # print("pred boxes3d selected", pred_boxes3d_selected)
            cur_sample_id = sample_id[k]
            # print(cur_sample_id) # 1,2,4,5,6,8,...
            calib = dataset.get_calib(cur_sample_id)
            final_total += pred_boxes3d_selected.shape[0]
            # print("final total", final_total) # increasing
            image_shape = dataset.get_image_shape(cur_sample_id)
            # print(image_shape) # 왜 image size가 바뀌지?
            save_kitti_format(cur_sample_id, calib, pred_boxes3d_selected, final_output_dir, scores_selected, image_shape, class_pred_selected)
