...
class Trainer(object):
    ...
    def _train_it(self, batch):
        ...
        # dict_keys(['sample_id', 'random_select', 'aug_method', 'pts_input', 'pts_rect', 'pts_features', 'gt_boxes3d'])
        self.model.train()
        self.optimizer.zero_grad()
        loss, tb_dict, disp_dict = self.model_fn(self.model, batch)
        # print("1",loss) # tensor(7.2680, device='cuda:0', grad_fn=<AddBackward0>)
        # print("2",tb_dict)
        # {'loss_x_bin': 1.7924153804779053, 'loss_z_bin': 1.7915948629379272, 'loss_x_res': 0.045637838542461395, 
        # 'loss_z_res': 0.059599850326776505, 'loss_y_offset': 0.009236350655555725, 'loss_ry_bin': 2.1969964504241943, 
        # 'loss_ry_res': 0.1520439088344574, 'loss_loc': tensor(3.6985, device='cuda:0', grad_fn=<AddBackward0>), 
        # 'loss_angle': tensor(2.3490, device='cuda:0', grad_fn=<AddBackward0>), 
        # 'loss_size': tensor(0.0262, device='cuda:0', grad_fn=<SmoothL1LossBackward>), 
        # 'rcnn_loss_cls': 1.1419057846069336, 'rcnn_loss_reg': 6.126098155975342, 'rcnn_loss': 7.268003940582275, 
        # 'rcnn_loss_loc': 3.698484182357788, 'rcnn_loss_angle': 2.3490402698516846, 
        # 'rcnn_loss_size': 0.07857371866703033, 'rcnn_cls_fg': 86, 'rcnn_cls_bg': 125, 'rcnn_reg_fg': 127}
        # print("3",disp_dict) # {'reg_fg_sum': 127, 'loss': 7.268003940582275}
        loss.backward()
