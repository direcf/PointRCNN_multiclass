...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        if rcnn_cls.shape[2] == 1:
            ...
            print("raw_scores.size",raw_scores.size()) # size([1,100,1])

            norm_scores = torch.sigmoid(raw_scores)
            print("norm_scores",norm_scores) # value range (0~1)
            print("norm_scores.size",norm_scores.size()) # size([1,100,1])
        ...
        for k in range(batch_size):
            ...
            if cur_inds.sum() == 0:
                continue
            print("cur_inds", cur_inds, cur_inds.size()) #size([100]) included 0 or 1 tensor

            pred_boxes3d_selected = pred_boxes3d[k, cur_inds]
            print("1",pred_boxes3d_selected) # size([N,7])
            raw_scores_selected = raw_scores[k, cur_inds]
            print("2",raw_scores_selected) # size([N,1])
            norm_scores_selected = norm_scores[k, cur_inds]
            print("3",norm_scores_selected) # size([N,1])
